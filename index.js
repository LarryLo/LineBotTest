...
function df() {
  ...
  return sheet[Math.floor(Math.random()*GWSheet.length)];
}
function dfs() {
...
function dfr() {
  let sheet=['社團夥伴','粉絲','競爭對手','師弟','上司與部下','心之友','親子','戰友','夥伴','夫婦','孽緣','青梅竹馬','援軍','商業夥伴','裝傻和吐槽','依賴','同卵雙胞胎','指腹為婚','三角關係的兩個人','主治醫生和患者','信徒','被害者與加害者','必要的互相傷害','戀人','濱口跟有野','奇奇與蒂蒂','鬍子和巨乳','光美跟少女','左乳與右乳','我是你你是我','三角木馬關係的兩個人','被抓住弱點的奴隸','烏賊・腳腳','二次元和三次元','富豪跟保鑣','魔王與勇者'];
  return sheet[Math.floor(Math.random()*GWSheet.length)];
}
////死亡flag function 結束
////雜項
