/* jshint esversion: 9 */
class Bot {
	constructor(...diceExtant) {
		var version = this.version = '2.1.1 cook-ng';
		//表格放置區
		////sw2.0
		var powerSheet = this.powerSheet = [
			[0, 0, 0, 1, 2, 2, 3, 3, 4, 4],
			[0, 0, 0, 1, 2, 3, 3, 3, 4, 4],
			[0, 0, 0, 1, 2, 3, 4, 4, 4, 4],
			[0, 0, 1, 1, 2, 3, 4, 4, 4, 5],
		...
		diceExtant.forEach(ex => {
			Object.getOwnPropertyNames(ex.prototype).forEach(method => {
				if (method !== 'constructor'){
					this[method] = (...params) => ex.prototype[method].call(this, ...params);
				} else if (method === 'constructor'){
					for (const [key, value] of Object.entries(new ex.prototype[method]())) {
						this[key] = value;
					}
				}
			});
